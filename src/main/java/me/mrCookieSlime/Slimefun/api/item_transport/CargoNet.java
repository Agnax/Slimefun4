...
public class CargoNet extends Network {
	...
	private static final ItemStack terminal_noitem_item = new CustomItem(new ItemStack(Material.BARRIER), "&4Ningún artículo almacenado en caché");
	...
	public void tick(Block b) {
		...
		if (connectorNodes.isEmpty() && terminusNodes.isEmpty()) {
			SimpleHologram.update(b, "&cNodos de carga no encontrados");
		}
		else {
					...
					for (Location l : terminals) {
						...
						for (int i = 0; i < terminal_slots.length; i++) {
							...
							if (items.size() > i + (terminal_slots.length * (page - 1))) {
								...
								lore.add("");
								lore.add(ChatColors.color("&7Items almacenados: &r" + DoubleHandler.getFancyDouble(item.getAmount())));
								
								if (stack.getMaxStackSize() > 1) lore.add(ChatColors.color("&7<Click izquierdo: Solicitar 1 | Click Derecho: Solicitar " + (item.getAmount() > stack.getMaxStackSize() ? stack.getMaxStackSize(): item.getAmount()) + ">"));
								else lore.add(ChatColors.color("&7<Click izquierdo: Solicitar 1>"));
